...
fs.readdir(dirPath, (err, files) => {
  ...
  files.forEach(file => {
    if (path.extname(file) === '.md') {
      ...
      const readMe = fs.readFileSync(filePath, 'utf-8'); // Explicit UTF-8 encoding
      ...
      <head>
        ...
        <style>
          ...
          .code-snippet {
            margin-top: -40px; // THIS IS FOR MOVING THE CHUNKS UP. Use a negative value to pull the code block up
          }
        </style>
      </head>
      <body>
      ...
      fs.writeFileSync(newFilePath, styledHtmlContent, 'utf-8'); // Explicit UTF-8 encoding
    }
  });
});
